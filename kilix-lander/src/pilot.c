/* The neural pilot: a kilix-game-kit policy compiled in from
 * src/neural_policy_blob.h (tools/neural/install-policy.py regenerates it),
 * and the per-tick hand-off between the keyboard and a computer pilot. */
...
int pilot_neural_action(void)
{
    ...
    if (!pilot_neural_ready()) return 0;
...
{
    if (G.state != GS_PLAYING || G.flying == PILOT_YOU) return false;
    if (G.flying == PILOT_NEURAL && pilot_neural_ready()) {
        game_apply_action(pilot_neural_action());   /* every tick, as in training */
        return true;
    }
    /* AUTOPILOT, or NEURAL without a usable policy: the scripted pilot. */
