...
MODEL.raw is brokeout-lab's float32 parameter file (KXPOLICY order). This packs
it with kilix-game-kit's own tools/kilix_policy.py into
assets/policy/brokeout-neural.kxpol, regenerates src/neural_policy_blob.h with
...
def main():
    ...
    manifest = {
        "artifact": "assets/policy/brokeout-neural.kxpol",
        ...
        "license": "MIT, same as kilix-brokeout",
        "origin": ("Self-trained in kilix-brokeout's own simulation (tools/neural/brokeout_lab.c "
                   "linked against game.o). No third-party data or weights."),
        "inputs": "game_policy_features() in src/game.c: 38 screen-independent floats",
        "outputs": "9 logits: where on the paddle face to strike the most urgent ball (far left .. far right); argmax each tick; game_apply_action moves the paddle there and launches a resting ball after 0.35 s",
    }
    if a.manifest_extra:
